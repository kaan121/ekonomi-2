...
if(args[0] == "bitcoin") {
    ...
    if(para < 10000) return message.channel.send(embed)
    if(para > 10000) {
        db.math(`para_${message.author.id}`,"-", 10000)
        db.add(`bitcoin_${message.author.id}`, 1)
        const embed = new discord.MessageEmbed()
        .setTitle('Satın alım başarılı!')
        .setDescription(`${message.author.username} adlı kişi 10000  :coin:  vererek bitcoin satın aldı!`)
    return message.channel.send(embed)
    }
}
}
